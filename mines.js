$(function () {
    ...
    function CutCalPos() {  
        id = Pos2Cell(TOUCH_INFO.current_x, TOUCH_INFO.current_y  )
    ...
    $('.wrapper').mousedown(function(event) {
        ...
        TOUCH_INFO.current_x = event.clientX;
        TOUCH_INFO.current_y = event.clientY;
        TOUCH_INFO.start_x = TOUCH_INFO.current_x;
        TOUCH_INFO.start_y = TOUCH_INFO.current_y;
        // CUT_START_CELL = this.id  // 不知道原因，第二次執行時，有可能會錯 
        // 因為採自行計算   $('.cell') => $('.wrapper')
        CUT_START_CELL =  Pos2Cell(TOUCH_INFO.current_x, TOUCH_INFO.current_y)

        console.log("this.id " + this.id)
        console.log("Pos2Cell " + Pos2Cell(TOUCH_INFO.start_x, TOUCH_INFO.start_y))
        console.log("mousedown " + TOUCH_INFO.start_x +"," + TOUCH_INFO.start_y)
    ...
    $('.wrapper').on('touchstart', function(event){
        if (event.touches && event.touches.length) {
            ...
            // 因為採自行計算   $('.cell') => $('.wrapper')
            CUT_START_CELL =  Pos2Cell(TOUCH_INFO.current_x, TOUCH_INFO.current_y)
            //console.log("touchstart " + TOUCH_INFO.start_x +"," + TOUCH_INFO.start_y)
        }
